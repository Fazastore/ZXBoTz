...
let handler = async(m, { conn, usedPrefix, command, text }) => {
  let owned = '6285774869896@s.whatsapp.net'
  if(!text) throw `*Cara penggunaan :*\n\n${usedPrefix + command} id|nomor\n\n*Note:*\n*Contoh:* ${usedPrefix + command} 085774869896|085774869896`;
  let [id, num] = text.split('|');
  let res = await fetch(`https://saipulanuar.ga/api/topup/epep6?id=${id}&nomor=${num}&apikey=${bear}`)
...
handler.help = ['topup720dm']
handler.tags = ['internet']
handler.command = /^(topup720dm)$/i
